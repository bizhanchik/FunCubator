...
struct ARViewContainer: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> ARView {
        ...
        massageImgEntity.position = SIMD3(0, 0.2, -1)
        ...
        massageTextEntity.position = SIMD3(-massageCenterOffsetX, -0.15, -1)
        ...
        ceoImgEntity.position = SIMD3(0, 0.2, -1)
        ...
        ceoTextEntity.position = SIMD3(-ceoCenterOffsetX, -0.15, -1)
        ...
        mentorImgEntity.position = SIMD3(0, 0.2, -1)
        ...
        mentorTextEntity.position = SIMD3(-mentorCenterOffsetX, -0.15, -1)

        mentorGroup.addChild(mentorImgEntity)
        mentorGroup.addChild(mentorTextEntity)
        mentorGroup.position = SIMD3(0, 0.7, -0.8) // Позади и выше других кнопок
...
class RoastScene: SKScene, SKPhysicsContactDelegate {
    ...
    private let questions = [
        "А CI/CD настроил?🤨",
        "А как ИИ интегрируешь⁉️",
        "А юнит-тесты где? 😏",
        "У тебя сервер упал🤬",
        "Почему нет аналитики? 😂",
        "Почему видео не виральны?😞",
        "Тиктоки снимаешь⁉️",
        "Аккаунт в Threads ведёшь?😏",
        "Когда ревенью?💰",
        "Что по продвижени?📱",
        "Почему юзеры не растут?🤨",
        "Почему не питчишься?",
        "Почему не продаешь?",
    ...
    private func createBoss(_ currentTime: TimeInterval) {
        let bossNode = BossQuestionNode(text: "Где прогресс????")
